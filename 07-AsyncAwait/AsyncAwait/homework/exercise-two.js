...
//version mejorada
  //  const poemNamesArr=['poem-two/stanza-01.txt','poem-two/stanza-02.txt']//genramos el array con los archivos
  //  const poemPromisesArr=poemNamesArr.map((file)=>promisifiedReadFile(file))//los mapeamos en un arreglo
  //  const promeses= await  Promise.all(poemPromisesArr)//se retorna las promesas 
 
//version ultra instinto 
const promeses= await  Promise.all(['poem-two/stanza-01.txt','poem-two/stanza-02.txt'].map((file)=>promisifiedReadFile(file)))

  for (let promes of promeses) {//for of obcjets
      blue(promes);
      
    }
  console.log('-- A. ASYNC version done --')
...
async function problemB () {
  ...
  // AsyncAwait version
  const promeses= await  Promise.all(filenames.map((file)=>promisifiedReadFile(file)));
